...
void UAttackDecisionComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    for (int i = 0 ;  i < arrDecisionPoint.Num() ; ++i)
    {
...
void UAttackDecisionComponent::PerformSweep(int Index, FCollisionShape Collision)
{
    ...
    for (const FHitResult& Hit : HitResults)
    {
        ...
        if (TargetChar)
        {
            ...
            TargetChar->ReceiveAttack(DataInfo, HitDirection);
        }
    }
}
